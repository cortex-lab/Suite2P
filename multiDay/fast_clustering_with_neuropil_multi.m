% function [ops, stat, res] = fast_clustering_with_neuropil_multi(ops, U, Sv)
...
% res.M = zeros(512);
% res.iclust = -ones(512);
% res.S = zeros(512, 512, size(S,2));

% Ly = numel(ops.yrange);
% Lx = numel(ops.xrange);
% res.M(ops.yrange, ops.xrange)       = reshape(M, Ly, Lx);
% res.iclust(ops.yrange, ops.xrange)  = reshape(iclust, Ly, Lx);
% res.S(ops.yrange, ops.xrange, :)    = S;
%
res.Ly  = Ly;
res.Lx  = Lx;
stat    = get_stat(res);
