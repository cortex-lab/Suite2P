function varargout = gui2p(varargin)
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @gui2p_OpeningFcn, ...
                   'gui_OutputFcn',  @gui2p_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before gui2p is made visible.
function gui2p_OpeningFcn(hObject, eventdata, h, varargin)
% This function has no output args, see OutputFcn.
% varargin   command line arguments to gui2p (see VARARGIN)
...
h.output = hObject;
guidata(hObject, h);
% UIWAIT makes gui2p wait for user response (see UIRESUME)
% uiwait(h.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = gui2p_OutputFcn(hObject, eventdata, h) 
...
pb = [84 93 101 86 87 89 90 92 103 98 95 96 99 100 104 112]; % 102
for j = 1:numel(pb)
...
h.statstr = 'npix';
h.statstrs = {'npix','compact','aspect_ratio','skew','std','footprint'};
h.statnum = 1;
...
if isfield(ops, 'mimg1') && ~isempty(ops.mimg1)
    h.dat.mimg(:,:,h.dat.maxmap) = ops.mimg1;
...
if isfield(ops, 'Vcorr') && ~isempty(ops.Vcorr)
    h.dat.mimg(ops.yrange,ops.xrange,h.dat.maxmap) = ops.Vcorr;
    h.dat.mimg_proc(ops.yrange,ops.xrange,h.dat.maxmap) = normalize_image(ops.Vcorr);
end
...
%
icell0 = double([h.dat.stat.iscell]);
%
statLabels  = h.statLabels;
% prior       = h.prior;
st          = cat(1, h.st, h.st0);
icell = cat(2, h.icell, icell0);
save(h.dat.cl.fpath, 'st', 'statLabels', 'icell')
...
pb = [98 95 96 99 100 104 112];  %  102
...
hval0            = min(3, [h.dat.stat.compact]);
hval             = hval0;
hval             = hval - min(hval);
hval             = hval / nanmean(hval);
hval             = .6 * (1 - min(2*nanstd(hval)+1, hval)/(nanstd(hval)*2 + 1)) + .15;
...
if FileName
    def_name = fullfile(rootS2p, 'cl_base.mat');
    load(def_name);    
    save(fullfile(PathName, FileName), 'st', 'statLabels', 'icell')
