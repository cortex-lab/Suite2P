...
for i = 1:numPlanes
    ...
    if ~ischar(ops1{i}.splitBlocks)
        yB = cumsum([0 ops1{i}.splitBlocks{1}]);
        xB = cumsum([0 ops1{i}.splitBlocks{2}]);
        
        ib = 0;
        for iy = 1:length(yB)-1
            for ix = 1:length(xB)-1
               ib = ib+1;
               ops1{i}.yBL{ib} = (yB(iy)+1):yB(iy+1);
               ops1{i}.xBL{ib} = (xB(ix)+1):xB(ix+1);
...
%                 data = img.loadFrames(fs{k}(j).name, ichanset(1), ichanset(2), ichanset(3));                
            ...
            while ix0<size(data,3)
                ...
                dreg(:, :, indxr)        = ...
                    blockRegisterMovie(data(:, :, indxr), ops1{1}, dsall(indxr,:,:));
                ix0 = ix0 + Nbatch;
            end
